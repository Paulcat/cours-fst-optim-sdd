...
% figures
\usepackage[inkscapeversion=1.3.1]{svg}
%\svgpath{{figs/}}
...
%\begin{itemize}[label=\tiny$\blacksquare$]
	...
	Equation de l'hyperplan: $\abold^\top \xx + b = 0$
...
\begin{itemize}[label=\tiny$\blacksquare$]
	...
	\eq{
		\min f(x) \qscq g_i(x) = 0, \quad i=1,\ldots,q
	}
...
\begin{itemize}[label=\tiny$\blacksquare$]
	...
	\eq{
		\min f(x) \qstq \left\{
		\begin{aligned}
			&g_i(\xx) = 0 \quad i=1,\ldots,q\\
			&h_j(\xx) \leq 0 \quad j=1,\ldots,p
		\end{aligned}\right.
	}
	...
	\eq{\tcbhighmath{
		\Ll(\xx,\lla,\nnu) \eqdef f(\xx) + \sum_{i=1}^q \nu_i g_i(x) + \sum_{j=1}^p \la_j h_j(\xx)
	}}
	...
	\eq{\left\{
		\begin{aligned}
			...
			&\tcbhighmath[colback=green!20,top=-2pt,bottom=-2pt]{g_i(\xx) = 0 \quad i=1,\ldots,q} \\
			&\tcbhighmath[colback=red!20,top=-2pt,bottom=-2pt]{\la_{j} \geq 0 \quad j=1,\ldots,p}\\
			&\tcbhighmath[colback=red!20,top=-2pt,bottom=-2pt]{\la_{j}h_j(\xx) = 0 \quad j=1,\ldots,p}
		\end{aligned}\right.
	}
...
\begin{itemize}[label=\tiny$\blacksquare$]
	...
	\eq{
		\min \frac{1}{2}\norm{\abold}^2 \qscq  y_j(\abold^\top \xx_j + b) \geq 1, \quad j=1,\ldots,p
	}

	\item C'es un problème \alert{convexe}. On considère le \alert{Lagrangien}
	\eq{\tcbhighmath{
		\Ll(\abold,b,\bm{\lla}) = \frac{1}{2}\norm{\abold}^2 + \sum_{j=1}^p\la_j(1-y_j(\abold^\top \xx_j + b))
	}}
...
\begin{itemize}[label=\tiny$\blacksquare$]
	...
	\eq{\tcbhighmath{\begin{aligned}
		\max_{\lla} \quad &\sum_{j=1}^p \la_j - \frac{1}{2}\sum_{j,j'=1}^p \la_j\la_{j'}y_jy_{j'}\xx_j^\top\xx_{j'} \\
		\qscq
			&\sum\la_j y_j = 0, \quad \la_j \geq 0, \quad j=1,\ldots,p
		\end{aligned}}
	}
...
\item La fonction de décision pour le problème de classification est finalement donnée par
	\eq{\tcbhighmath{
		\sgn\left(\sum_{j=1}^p\la_{j*}y_j \dotp{\xx}{\xx_j} + b_*\right)
	}}
\end{itemize}
...
\begin{itemize}[label=\tiny$\blacksquare$]
	...
	\eq{\tcbhighmath{
		\min \frac{1}{2}\norm{\abold}^2 + \al\sum_{j=1}^p \xi_j \qscq  y_j(\abold^\top \mathbf{\alert{\phi}}(\xx_j) + b) \geq 1 - \xi_j, \quad \xi_j \geq 0, \quad j=1,\ldots,p
	}}
	
	\item Problème dual
		\eq{\tcbhighmath{\begin{aligned}
		\max_{\lla} \quad &\sum_{j=1}^p \la_j - \frac{1}{2}\sum_{j,j'=1}^p \la_j\la_{j'}y_jy_{j'}\dotp{\alert{\phi}(\xx_j)}{\alert{\phi}(\xx_{j'})}\\
		\qscq
			&\sum\la_j y_j = 0, \quad \al \geq \la_j \geq 0, \quad j=1,\ldots,p
		\end{aligned}}
	}
...
\begin{itemize}[label=\tiny$\blacksquare$]
	\item L'enjeu est de définir un noyau $K(\xx_j,\xx_{j'}) = \dotp{\phi(\xx_j)}{\phi(\xx_{j'})}$ (un noyau est simplement une fonction symétrique de $\Xx \times \Xx$ dans $\RR$) tel que
	\begin{itemize}[label=-]
