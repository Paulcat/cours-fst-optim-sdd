...
\subtitle{Sciences des données -- Décompositions matricielles et tensorielles}
...
\date{paul.catala(at)univ-lorraine.fr \hfill Semestre 9 -- 2024 \\ \\ Basé sur des cours de D. Brie, S. Miron et K. Usevich}
...
\begin{itemize}[label=\tiny$\blacksquare$]
	\item Données complexes, nombreuses, en grande dimension: images, vidéos, sons, champs électriques ou magnétiques, textes, graphes relationnels, etc...
	
	\vfill
	\begin{tcolorbox}
	Ce qu'il faut attendre de ce cours: pour un certain type de données...
	
	\item \alert{\textbf{... comment représenter efficacement ces données?}}
	
	$\rightarrow$ Modèles de complexité faible (\eg\; faible rang, parcimonie)
	
	\textcolor{mLightBrown!20}{$\rightarrow$} Réduction de dimension
	
	\vspace{0.5cm}
	
	\item \alert{\textbf{... quelles méthodes pour traiter ces données?}}
	
	
	$\rightarrow$ Régression, classification
	\end{tcolorbox}
\end{itemize}
...
\section[Données et décompositions]{Données et décompositions matricielles}
...
\begin{frame}
\frametitle{Matrices dans l'analyse de données}

\begin{itemize}[label=\tiny$\blacksquare$]
	\item \alert{Données sous forme matricielle} :
	\begin{itemize}[label=-]
		\item Signaux continus / données temporelles $\vect{x}(t)$ 
		\[
		\includegraphics[width=0.4\textwidth]{figures/melang.eps}
		\] 
		\item Images en niveaux de gris / discrétisation d'une fonction $f(x,y)$
		\item $N$ échantillons, $p$ variables explicatives : matrice $N \times p$
	\end{itemize}

	\item \alert{Matrices à partir des données} :
	\begin{itemize}[label=-]
		\item Statistiques des données: matrice de covariance $\mathbf{E}(\vect{x} \vect{x}^{T})$, histogrammes 2D
		\item Matrices d'adjacence / laplacienne pour des données sur graphes
	\end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Exemple de matrices faible rang}

\begin{figure}
	\subfloat[rang = \only<2->{1}]{\includegraphics[width=0.3\textwidth]{figures/flag-austria}}
	\;\;\vrule\;
	\subfloat[rang = \only<3->{2}]{\includegraphics[width=0.3\textwidth]{figures/flag-denmark}}
	\;\;\vrule\;
	\subfloat[rang = \only<3->{3}]{\includegraphics[width=0.3\textwidth]{figures/flag-greece}}
\end{figure}
\end{frame}
...
\begin{tcolorbox}
\textbf{SVD (``economy size'')}
$$
\mathbf{X} = \mathbf{U} \, \bm{\Sigma} \, \mathbf{V}^T = \sum_{i=1}^p \sigma_i \, \mathbf{u}_i \,  \mathbf{v}_i^T
$$

\[
\raisebox{-0.35\height}{\begin{tikzpicture}[scale=0.9]
...
\draw(-0.6,1.65) node (eq) {\footnotesize ${p}$};
...
\draw(0.98,0.65) node (S) {$\mathbf{\sigma_p}$};
\draw(0,1) node (eq) {\footnotesize ${p}$};\\
\draw(0.7,1.65) node (eq) {\footnotesize ${p}$};


\draw(1.65,0.5) rectangle (3.5,1.5);
\draw(1.45,0.95) node (eq) {\footnotesize ${p}$};
...
$\mathbf{X}$ : $(m \times n)$  matrice de données\\[1ex]

$\mathbf{U} = [\mathbf{u}_1 | \dots | \mathbf{u}_p ]$ : $(m \times p)$  matrice orthogonale (vecteurs singuliers gauches)\\[1ex]

$\mathbf{V} = [\mathbf{v}_1 | \dots | \mathbf{v}_p ]$ : $(n\times p)$ matrice orthogonale (vecteurs singuliers droits)\\[1ex]

 $\bm{\Sigma} = \text{diag}\{\sigma_1, \dots,  \sigma_p \}$ :  $(p \times p)$   matrice diagonale\\[2ex] 

$\sigma_1 \geq \dots \geq  \sigma_p \geq 0$ : valeurs singulières\\[1ex] 
...
\begin{figure}
	...
	\subfloat[rang=2]{\includegraphics[width=0.3\textwidth]{figures/flag-denmark}}
	\;
	\subfloat[rang=3]{\includegraphics[width=0.3\textwidth]{figures/flag-greece}}
	\;
	\subfloat[rang=?]{\includegraphics[height=0.225\textwidth]{figures/qrcode}}
	\\
	\subfloat[rang=2]{\includegraphics[width=0.3\textwidth]{figures/svd-denmark}}
	\;
	\subfloat[rang=3]{\includegraphics[width=0.3\textwidth]{figures/svd-greece}}
	\;
	\subfloat[rang=?]{\includegraphics[width=0.3\textwidth]{figures/svd-qrcode}}
\end{figure}
...
\begin{itemize}[label=\tiny$\blacksquare$]
	...
	\item Les \alert{valeurs singulières} (non-nulles) de $\mathbf{X}$ sont les racines carrées des valeurs propres (non-nulles) de $\mathbf{X}^T  \mathbf{X}$ ou $\mathbf{X} \,  \mathbf{X}^T$.\\[3ex]
\end{itemize}
...
\begin{frame}
	...
	\frametitle{Propriétés de la SVD}
	
	\begin{itemize}[label=\tiny$\blacksquare$]
	\item La SVD est \textbf{unique} (sous certaines conditions).\\[3ex]
	
	\item La norme de Frobenius de $\mathbf{X}$ : $\left\lVert \mathbf{X} \right\rVert_F= \sqrt{\sigma_1^2 +\dots +\sigma_p^2} .$\\[3ex]
	
	\item La SVD donne la "meilleure" approximation de rang faible d'une matrice: 
	\begin{tcolorbox}	
	la solution du problème d'optimisation
	
	$$
	\min\left\lVert \mathbf{X} - \mathbf{X}_r \right\rVert_F \quad \text{s. c.} \quad \text{rang}( \mathbf{X}_r) =r, \quad r<p
	$$
...
\[
\begin{array}{c@{}l}
\text{\uncover<4->{\raisebox{-0.45\height}{\includegraphics[height=1.2cm]{figures/2dspectrum1}} = }} & 
\raisebox{-0.35\height}{\begin{tikzpicture}
...
\text{\uncover<4->{{\raisebox{-0.45\height}{\includegraphics[height=1.2cm]{figures/2dspectrum2}}} = }} & 
...
\raisebox{-0.5\height}{\includegraphics[height=1.6cm]{figures/conv_full.png}} &\to& \raisebox{-0.5\height}{\includegraphics[height=1.6cm]{figures/conv_cpd.png}}
\end{array}
\]
